...
function renderDirChildren(dir, parent, matcher, folderPaths, excludePaths = []) {
  ...
  entries.forEach((node) => {
    ...
    row.addEventListener("click", async (event) => {
      ...
      els.sidebarPanel?.querySelectorAll(".tree-row.active").forEach((el) => el.classList.remove("active"));
...
function rememberMindmapNodeForEditing(node) {
  const uid = node?.getData?.("uid");
  if (!uid) return;
  state.mindmapKeyCaptureActive = true;
  state.mindmapLastClickedNodeUid = uid;
}
